"""
改善版: ArduPilotパラメータ設定スクリプト (No2/Futaba向け EKF Observer v5)
setup_futaba_4.py の双葉ハードウェア設定（UART 921600, 双葉レシーバー, RC設定）を維持しつつ、
setup_EKF_Observer3.py の最新 Observer/EKF パラメータ・PID・WPNAV 値を反映した版です。

主な変更点（v4 → v5）:
1. Observer/EKF パラメータを setup_EKF_Observer3.py の最新値に更新
   - OBS_CORR_GAIN: 0.0 → 0.1
   - OBS_MAX_CORR_ANG: 0.873 → 0.5
   - OBS_PRED_TIME: 0.01 → 0.00
   - OBS_EKF_Q_D:   9.5e-12   → 9.5367432e-3
   - OBS_EKF_Q_DD:  2.4e-11   → 2.3841858e-3
   - OBS_EKF_Q_C:   4.8e-13   → 4.7683716e-3
   - OBS_EKF_INN_MAX: 0.7 → 1.0
   - OBS_W_FREEZE (新規), OBS_FADE_TH/DLY/IN_T/OUT_T (新規)
2. 水平制御PID を EKF3 最新値に更新
   - PSC_POSXY_P: 5.0 → 1.0
   - PSC_VELXY_P: 3.0 → 2.0
   - PSC_VELXY_I: 2.5 → 1.0
3. 姿勢制御PID を EKF3 最新値に更新
   - ATC_RAT_RLL_P: 0.048 → 0.10, ATC_RAT_RLL_D: 0.00144 → 0.0012
   - ATC_RAT_PIT_P: 0.06  → 0.10, ATC_RAT_PIT_D: 0.00144 → 0.0012
4. WPNAV を EKF3 最新値に更新（小さな円に適した速度）
   - WPNAV_SPEED:  500 → 100 cm/s
   - WPNAV_ACCEL:  500 → 200 cm/s^2
   - WPNAV_RADIUS:   5 →  50 cm
5. LOG_BITMASK, MOT_THST_HOVER, MOT_THST_EXPO, MOT_HOVER_LEARN を追加
6. 双葉固有設定は変更なし（UART 921600, SERIAL2_PROTOCOL=2, RC設定, RSSI_TYPE=0）
"""
...
params_to_set = {
    ...
    # === 周波数範囲設定（EKF3 最新値: L=1.04m FFT平均ベース） ===
    ...
    # === EKF ゲーティング / イノベーション制限（EKF3 最新値） ===
    'OBS_EKF_INN_MAX':  (1.0, 'AP_Float', '統合周波数更新に軸を含めるための最大絶対イノベーション [N]'),

    # === 出力フェード制御（EKF3 最新値: メインスイッチ・離陸トリガー・不感帯統合） ===
    ...
    # === シリアル設定（双葉: UART 921600, SERIAL2=双葉レシーバー） ===
    ...
    # 他のスイッチ設定を一旦クリアする
    'RC5_OPTION': (0, 'AP_Int16', '重複回避用: Do Nothing'),
    'RC9_OPTION': (0, 'AP_Int16', '重複回避用: Do Nothing'),
    'RC10_OPTION': (0, 'AP_Int16', '重複回避用: Do Nothing'),
    'RC11_OPTION': (0, 'AP_Int16', '重複回避用: Do Nothing'),
    'RC12_OPTION': (0, 'AP_Int16', '重複回避用: Do Nothing'),
...
print("\n" + "=" * 60)
print("No2/Futaba + EKF Observer v5 パラメータを設定中...")
print("=" * 60)
...
if final_failed:
    ...
    if eeprom_saved:
        ...
        print("   （No2/Futaba v5版、Observer/EKFパラメータはEKF3最新値）")
    else:
