"""
改善版: ArduPilotパラメータ設定スクリプト (No2/Futaba向け EKF Observer版)
主な改善点：
...
EKF/setup_EKF_Observer.py の設定をベースに、No2 の Futaba ハードウェアで
そのまま使えるように接続設定と RC/シリアル設定を追加した版です。
"""
...
def wait_for_param_ack(param_name, timeout=2.0):
    ...
    while time.time() - start_time < timeout:
        ...
        if msg_param_name == param_name:
            return msg_dict.get('param_value', None), True
    return None, False
...
def set_parameter_reliable(param_name, param_value, param_type, max_retries=5):
    ...
    for attempt in range(max_retries):
        ...
        received_value, received = wait_for_param_ack(param_name, timeout=1.5)
...
print("\n" + "=" * 60)
print("No2/Futaba + EKF Observer パラメータを設定中...")
print("=" * 60)
