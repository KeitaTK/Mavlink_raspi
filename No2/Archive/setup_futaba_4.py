"""
改善版: ArduPilotパラメータ設定スクリプト (No2/Futaba向け EKF Observer v4)
主な改善点：
...
No2 の Futaba ハードウェア設定は維持しつつ、
EKF/setup_EKF_Observer3.py の Observer/EKF 拡張パラメータを取り込んだ版です。
"""
...
params_to_set = {
    ...
    # === 姿勢制御PID ===
    'ATC_RAT_RLL_P': (0.048, 'AP_Float', 'Roll P'),
    'ATC_RAT_RLL_I': (0.05, 'AP_Float', 'Roll I'),
    'ATC_RAT_RLL_D': (0.00144, 'AP_Float', 'Roll D'),
    'ATC_RAT_PIT_P': (0.06, 'AP_Float', 'Pitch P'),
    'ATC_RAT_PIT_I': (0.05, 'AP_Float', 'Pitch I'),
    'ATC_RAT_PIT_D': (0.00144, 'AP_Float', 'Pitch D'),
    'ATC_RAT_YAW_P': (0.2, 'AP_Float', 'Yaw P'),
    'ATC_RAT_YAW_I': (0.02, 'AP_Float', 'Yaw I'),

    # === Observer設定 ===
    'OBS_CORR_GAIN': (0.0, 'AP_Float', 'オブザーバ補正ゲイン (0.0-1.0)'),
    'OBS_FILT_CUTOFF': (30.0, 'AP_Float', 'フィルタカットオフ周波数 [Hz]'),
    'OBS_MAX_CORR_ANG': (0.873, 'AP_Float', '姿勢補正の最大角度 [rad] (~50度)'),
    'OBS_PRED_TIME': (0.01, 'AP_Float', '外力予測の先読み時間 [秒]'),
    ...
    'OBS_EKF_W_INIT': (obs_dist_freq, 'AP_Float', 'EKFの初期角周波数 [rad/s]'),
    'OBS_EKF_W_MIN': (2.2, 'AP_Float', 'EKFの最小角周波数 [rad/s]'),
    'OBS_EKF_W_MAX': (5.7, 'AP_Float', 'EKFの最大角周波数 [rad/s]'),
    'OBS_EKF_INN_MAX': (0.7, 'AP_Float', '周波数融合に使う軸の最大イノベーション [N]'),
    'OBS_EKF_NIS_MAX': (4.0, 'AP_Float', '周波数融合に使う軸の最大NIS'),
    'OBS_EKF_EN_GAT': (1, 'AP_Int8', 'RMSベースの軸ゲーティング有効化'),
    'OBS_EKF_EN_ON': (0.2, 'AP_Float', '周波数融合有効化のRMSしきい値 [N]'),
    'OBS_EKF_EN_OFF': (0.16, 'AP_Float', '周波数融合無効化のRMSしきい値 [N]'),
...
print("\n" + "=" * 60)
print("No2/Futaba + EKF Observer v4 パラメータを設定中...")
print("=" * 60)
...
if final_failed:
    ...
    if eeprom_saved:
        ...
        print("   （No2/Futaba版、OBS_* は拡張EKF Observer設定）")
    else:
