...
params_to_set = {
    ...
    # --- コンパス使用に変更：GPS位置+コンパスヨー角、速度はIMU推定 ---
    ...
    'EK3_SRC1_YAW': 1,       # Compass (ヨー角) ← GPS→Compassに変更[2][9]
    ...
    'EK3_YAW_M_NSE': 1.0,    # ヨー角ノイズ（コンパス用）
    
    # --- センサーノイズ調整 ---
    'EK3_ALT_M_NSE': 10.0,   # 気圧センサーノイズ
    'EK3_GYRO_P_NSE': 0.02,  # ジャイロプロセスノイズ（存在確認済み）[6][11]
    
    # --- コンパス設定（有効化） ---
    'COMPASS_ENABLE': 1,     # コンパス有効化（0→1）[2][9]
    'COMPASS_USE': 1.0,      # 内蔵コンパス使用（0→1）[9]
    'COMPASS_USE2': 0.0,     # 外付コンパス2無効
    'COMPASS_USE3': 0.0,     # 外付コンパス3無効
    'COMPASS_AUTODEC': 1,    # 自動磁気偏角有効[2]
    'COMPASS_LEARN': 1,      # コンパス学習有効[2]
    ...
    'ATC_RAT_YAW_P': 0.2,    # Yaw P（コンパス対応）
