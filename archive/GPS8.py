...
try:
    ...
    with open(output_csv, 'w', newline='') as f:
        ...
        # 補完用の最後の値保持変数
        last_gps = (0.0, 0.0, 0.0)
        last_ekf = (0.0, 0.0, 0.0)
        last_guided = (0.0, 0.0, 0.0)
        gps_set = False
        ekf_set = False
        guided_set = False
        ...
        for t in all_times:
            ...
            # GPSデータの補完
            if gps[0] != 0.0 or gps[1] != 0.0 or gps[2] != 0.0:
                last_gps = gps
                gps_set = True
            elif gps_set:
                gps = last_gps
            ...
            # EKFデータの補完
            if ekf[0] != 0.0 or ekf[1] != 0.0 or ekf[2] != 0.0:
                last_ekf = ekf
                ekf_set = True
            elif ekf_set:
                ekf = last_ekf
            ...
            # GUIDEDデータの補完
            if guided[0] != 0.0 or guided[1] != 0.0 or guided[2] != 0.0:
                last_guided = guided
                guided_set = True
            elif guided_set:
                guided = last_guided
