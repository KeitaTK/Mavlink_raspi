...
# ログファイルのパス
log_file = '/home/taki/Mavlink_raspi/log_analyzer/LOGS1/00000103.BIN'
output_csv = '/home/taki/Mavlink_raspi/log_analyzer/CSV/local_10.csv'
...
def lla_to_ned(lat, lon, alt):
    ...
    # ECEFからNEDに変換
    ned_x = -d_x * np.sin(lam) + d_y * np.cos(lam)  # East
    ned_y = -d_x * np.sin(phi) * np.cos(lam) - d_y * np.sin(phi) * np.sin(lam) + d_z * np.cos(phi)  # North
    ned_z = d_x * np.cos(phi) * np.cos(lam) + d_y * np.cos(phi) * np.sin(lam) + d_z * np.sin(phi)  # Down
