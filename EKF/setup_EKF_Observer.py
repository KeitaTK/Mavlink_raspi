"""
改善版: ArduPilotパラメータ設定スクリプト（EKF ベース Observer版）
主な改善点：
...
params_to_set = {
    ...
    'OBS_CORR_GAIN': (0.6, 'AP_Float', 'オブザーバ補正ゲイン (0.0-1.0)'),
    ...
    'OBS_EKF_Q_C': (0.01, 'AP_Float', 'EKFのDCオフセット状態cのプロセスノイズ分散'),
    'OBS_EKF_Q_W': (0.01, 'AP_Float', 'EKFの周波数状態omegaのプロセスノイズ分散'),
    'OBS_EKF_R_MEAS': (0.5, 'AP_Float', '外力観測値の観測ノイズ分散'),
    'OBS_EKF_W_INIT': (obs_dist_freq, 'AP_Float', 'EKFの初期角周波数 [rad/s]'),
    'OBS_EKF_W_MIN': (0.1, 'AP_Float', 'EKFの最小角周波数 [rad/s]'),
    'OBS_EKF_W_MAX': (10.0, 'AP_Float', 'EKFの最大角周波数 [rad/s]'),
    'OBS_PRED_TIME': (0.0, 'AP_Float', '外力予測の先読み時間 [秒]'),
    'OBS_MAX_CORR_ANG': (0.873, 'AP_Float', '姿勢補正の最大角度 [rad] (~50度)'),
    'OBS_EKF_INN_MAX': (5.0, 'AP_Float', '周波数融合に使う軸の最大イノベーション [N]'),
    'OBS_EKF_NIS_MAX': (10.0, 'AP_Float', '周波数融合に使う軸の最大NIS'),
    'OBS_EKF_EN_GAT': (1, 'AP_Int8', 'RMSベースの軸ゲーティング有効化'),
    'OBS_EKF_EN_ON': (0.5, 'AP_Float', '周波数融合有効化のRMSしきい値 [N]'),
    'OBS_EKF_EN_OFF': (0.3, 'AP_Float', '周波数融合無効化のRMSしきい値 [N]'),
    'OBS_EKF_EN_TAU': (1.0, 'AP_Float', 'RMS推定のEMA時定数 [s]'),
    ...
    'OBS_EKF_AX_MASK': (7, 'AP_Int8', '周波数融合に使う軸のビットマスク (0-7)'),
    'OBS_EKF_RB_EN': (1, 'AP_Int8', 'ロバスト観測更新（外れ値除去）有効化'),
    'OBS_EKF_RB_NIS': (2.0, 'AP_Float', 'NIS外れ値除去の乗数'),
    'OBS_EKF_HOLD_W': (0.5, 'AP_Float', 'omega更新保持時の軸融合重み'),
    'OBS_EKF_SH_BETA': (0.5, 'AP_Float', 'XY軸のshared omega注入ブレンド係数'),
    'OBS_EKF_SH_HWM': (1.0, 'AP_Float', 'hard shared-omegaモード突入の最小融合重み'),
    'OBS_EKF_SH_NIS': (10.0, 'AP_Float', 'hard shared-omegaモード有効化の最大NIS'),
