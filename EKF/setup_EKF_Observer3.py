# """
# 改善版: ArduPilotパラメータ設定スクリプト（EKF ベース Observer版 + RCスイッチ対応）
# 主な改善点：
...
# # 変更履歴
# # ================================
# # 2026-06-07: ペイロード円飛行/四角形飛行向けパラメータ最適化
# #   [変更内容]
# #   1. WPNAV_SPEED:  500 → 100 cm/s   (半径0.3mの円に適した速度、1.0 m/s)
# #   2. WPNAV_ACCEL:  500 → 200 cm/s^2 (滑らかな加減速、2.0 m/s^2)
# #   3. WPNAV_RADIUS:   5 →  50 cm     (緩やかな到達判定、0.5 m)
# #   4. PSC_POSXY_P:  4.0 → 1.0        (デフォルト値、オーバーシュート防止)
# #   5. PSC_VELXY_P:  6.0 → 2.0        (デフォルト値、高ゲイン振動防止)
# #   6. PSC_VELXY_I:  2.5 → 1.0        (デフォルト値、積分飽和防止)
# #   7. PSC_VELXY_D:  1.0 → 0.5        (デフォルト値、ノイズ増幅防止)
# #   8. ATC_RAT_RLL_P: 0.04 → 0.10     (標準的小型クアッド値、姿勢制御強化)
# #   9. ATC_RAT_PIT_P: 0.05 → 0.10     (標準的小型クアッド値、姿勢制御強化)
# #  10. OBS_CORR_GAIN: 0.3 → 0.1       (小さな円での過剰補正防止)
# #  [変更理由] 詳細は本ファイル末尾の変更履歴コメント参照
