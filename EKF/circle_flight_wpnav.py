#!/usr/bin/env python3
"""
circle_flight_wpnav.py - WPNAV互換モードで円飛行を実行するスクリプト

circle_flight.py からの派生。
core 変更点: execute_circle_flight() を WPNAV（MAV_CMD_NAV_WAYPOINT 単発送信 +
到着確認 → 次WP）方式に置き換え。

旧: 10Hz で set_position_target_global_int を連続送信（連続軌道追従）
新: 各ウェイポイントを MAV_CMD_NAV_WAYPOINT で1回送信 → 到着確認 → 次WP
...
パラメータファイルは circle_params.json を共用。
num_waypoints デフォルト: 18（WPNAV向けに減らす）。
"""
...
def _validate_params(params):
    ...
    n_wp = params.get("num_waypoints", 18)
    if not (8 <= n_wp <= 360):
        raise ValueError(
...
class CircleFlightController:
    """
    Guidedモードで円飛行を実行するコントローラークラス（WPNAV 互換版）。
    ...
    execute_circle_flight() のみ WPNAV 方式（MAV_CMD_NAV_WAYPOINT 単発送信 +
    到着確認）に書き換え。その他のメソッドは circle_flight.py を踏襲。
    """
    ...
    # ──────────────────────────────────────────
    #  セットポイント送信（move_to_start_position / return_to_takeoff 用）
    # ──────────────────────────────────────────

    def _send_setpoint(self, lat_deg, lon_deg, alt_m, yaw_deg):
        """
        位置目標を MAVLink で送信する（set_position_target_global_int）。
    ...
    # ──────────────────────────────────────────
    #  WPNAV ウェイポイント送信（execute_circle_flight 用）
    # ──────────────────────────────────────────
    ...
    # ──────────────────────────────────────────
    #  execute_circle_flight（WPNAV 方式: コア変更点）
    # ──────────────────────────────────────────

    def execute_circle_flight(self):
        """
        円飛行のメインループを WPNAV 方式で実行する。

        旧方式（circle_flight.py）:
            10Hz で set_position_target_global_int を連続送信し、
            ドローンに連続軌道を追従させる。

        新方式（本メソッド）:
            各ウェイポイントを MAV_CMD_NAV_WAYPOINT で1回だけ送信し、
        ...
        """
        print(
            f"\n--- [{self.STATE_CIRCLE_FLYING}] 円飛行実行 (WPNAV) ---"
        )
        ...
        print(f"  WP数: {n_wp}（WPNAV 単発送信）")
        print(f"  WPNAV_RADIUS: {self.WPNAV_RADIUS}m | 推定時間: {estimated_total:.1f}秒")
        ...
        for lap in range(n_laps):
            ...
            for wp_idx in range(n_wp):
                ...
                # ── WPNAVウェイポイントを1回送信 ──
                self._send_wpnav_waypoint(lat_wp, lon_wp, alt_wp, yaw_wp)
                wp_send_count += 1
    ...
    def run(self):
        ...
        """
        print("=" * 60)
        print("  ArduPilot 円飛行制御 - Circle Flight (WPNAV)")
        print("=" * 60)
    ...
    def _save_csv(self):
        ...
        path = self.CSV_DIR / f"{now}_circle_wpnav.csv"
