"""
AP_Observer EKF対応: ArduPilotパラメータ設定スクリプト (No1用)
主な改善点：
1. メッセージバッファをクリア（古いメッセージ排除）
2. パラメータ名でメッセージをフィルタリング
3. ACK方式のタイムアウト処理
4. 段階的な設定と検証
5. Observer EKFパラメータ対応、RC7/RC8スイッチ設定追加
"""
...
params_to_set = {
    ...
    # === 姿勢・角速度・加速度制限 ===
    'ANGLE_MAX': (4500, 'AP_Int16', '最大傾斜角[0.01deg] (例:4500=45度)'),
    'ATC_ACCEL_R_MAX': (90000, 'AP_Int32', 'Roll最大加速度[deg/s^2] (例:90000)'),
    'ATC_ACCEL_P_MAX': (90000, 'AP_Int32', 'Pitch最大加速度[deg/s^2] (例:90000)'),
    'ATC_ACCEL_Y_MAX': (45000, 'AP_Int32', 'Yaw最大加速度[deg/s^2] (例:45000)'),
    ...
    'OBS_EKF_Q_D': (9.5367432e-12, 'AP_Float', 'EKFプロセスノイズ d'),
    'OBS_EKF_Q_DD': (2.3841858e-11, 'AP_Float', 'EKFプロセスノイズ d_dot'),
    'OBS_EKF_Q_C': (4.7683716e-13, 'AP_Float', 'EKFプロセスノイズ c'),
    'OBS_EKF_Q_W': (0.0005, 'AP_Float', 'EKFプロセスノイズ omega'),
    'OBS_EKF_R_MEAS': (46.0, 'AP_Float', 'EKF観測ノイズ'),
    'OBS_EKF_W_INIT': (obs_dist_freq * 2 * math.pi, 'AP_Float', 'EKF初期角周波数 [rad/s]'),
    'OBS_EKF_W_MIN': (2.1991, 'AP_Float', 'EKF最小角周波数 [rad/s]'),
    'OBS_EKF_W_MAX': (5.7180, 'AP_Float', 'EKF最大角周波数 [rad/s]'),
    'OBS_EKF_INN_MAX': (0.70, 'AP_Float', 'EKFイノベーション最大値 [N]'),
    'OBS_FADE_TH': (1.0, 'AP_Float', '出力フェード振幅閾値 [N]'),
    'OBS_FADE_DLY': (2.0, 'AP_Float', '出力フェード遅延 [s]'),
    'OBS_FADE_IN_T': (0.1, 'AP_Float', '出力フェードイン時定数 [s]'),
    'OBS_FADE_OUT_T': (1.0, 'AP_Float', '出力フェードアウト時定数 [s]'),
